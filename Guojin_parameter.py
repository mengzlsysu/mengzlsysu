# -*- coding: utf-8 -*-
"""
Created on Thu Dec 21 19:34:47 2017
...
##################### 特征寻优 ############################
N_list = [2,4,8,16,32,64,102]
Tree_list = [10,20,40,80,150]
...
for m in N_list:
    for t in Tree_list:
        ...
        ###################### 训练模型 ############################
            ...
            for i in feavec_sorted.feavec[0:m+1]:
            ...
            rf1=RandomForestClassifier(n_estimators = t) #遍历决策树参数的随机森林
        ...
        predict_accuracy_mean.append(np.mean(predict_accuracy))
        sample_accuracy_mean.append(np.mean(sample_accuracy))

predict_accuracy_mean = np.array(predict_accuracy_mean).reshape(len(N_list),len(Tree_list))
sample_accuracy_mean = np.array(sample_accuracy_mean).reshape(len(N_list),len(Tree_list))